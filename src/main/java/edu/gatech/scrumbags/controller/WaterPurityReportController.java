...
public class WaterPurityReportController {
    ...
    /** Initializes the WaterPurityReportView as to minimize errors by populating combo boxes, setting default choices and making
     * the error message invisible. */
    @FXML
    public void initialize () {
        WaterSourceReport sourceReport = MainFXApplication.getLastUsedSourceReport();
        locationLabel
            .setText(sourceReport.getLocation().getLatitudeString() + ", " + sourceReport.getLocation().getLongitudeString());
...
// datePickerBox.setValue(new LocalDate());
    ...
    /** Handles the input verification when the user presses the submit button. Checks that virusPPM and contaminantPPM are filled
     * out correctly. */
    @FXML
    public void handleSubmitPressed () {
        ...
        if (successfulParse) {
            WaterPurityReport purityReport = new WaterPurityReport(MainFXApplication.getLastUsedSourceReport(), reportDate,
            ...
            MainFXApplication.client.sendPurityReport(purityReport, MainFXApplication.getLastUsedSourceReport());
