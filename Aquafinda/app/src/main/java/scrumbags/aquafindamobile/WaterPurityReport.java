...
/** Water purity report class to M9 specification. Note: In this implementation a reference to an associated Source is held in
 * each WaterPurityReport, so location and Water type are accessible indirectly via getSource().
 * <p>
 * Specification from wiki:
 * <p>
 * Water Purity Report A real water report would need to address multiple bacteria/virus and chemical contaminants. For our basic
 * application, we assume that parts-per-million (PPM) is our measure and we will lump everything into two categories: Viruses and
 * Contaminants.
 * <p>
 * 1. Date and time of the report (can be autogenerated by application) 2. Report Number (must be autogenerated by application) 3.
 * Name of Worker (can be autogenerated from worker information) 4. Location of water (manually entered. using any kind of
 * location services or gps is extra credit). 5. Overall Condition (Safe / Treatable / Unsafe) 6. Virus PPM 7. Contaminant PPM
 * <p>
...
public class WaterPurityReport extends WaterReport {
	...
	public WaterPurityReport (WaterSourceReport source, Date purityReportDate, WaterPurityCondition purityCondition,
							  double virusPPM, double contaminantPPM, String submitterName) {
	...
	public String toString () {
		return super.toString() + ", Condition: " + getPurityConditionDescription() + ", Virus PPM: " + getVirusPPM()
				+ ", Contaminant PPM: " + getContaminantPPM();
	}
